...
/*
 return image from bundle file which is not chace. Default is png extension
 */
UIImage *imageWithContentOfFile(NSString *imageName, NSString *ext) {
    UIImage *image = [UIImage imageWithContentsOfFile:[[NSBundle mainBundle] pathForResource:imageName ofType:ext?ext:@"png"]];
    return image;
}
...
+ (CGSize)calculateHeightForText:(NSString *)text font:(UIFont *)font size:(CGSize)size {
    CGSize _size = [text sizeWithFont:font constrainedToSize:size lineBreakMode:NSLineBreakByWordWrapping];
    return _size;
}
