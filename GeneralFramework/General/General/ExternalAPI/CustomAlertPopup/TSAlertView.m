...
const CGFloat kTSAlertView_TopMargin	= 25.0;//16
const CGFloat kTSAlertView_BottomMargin = 15.0;
const CGFloat kTSAlertView_RowMargin	= 5.0;//5
...
- (UILabel*) titleLabel
{
	if ( _titleLabel == nil )
	{
        ...
        _titleLabel.font = [UIFont fontWithName:FONT_STYLE size:18];
		_titleLabel.backgroundColor = [UIColor clearColor];
		_titleLabel.textColor = [UIColor customYellowColor];
...
- (UILabel*) messageLabel
{
	if ( _messageLabel == nil )
	{
		...
		_messageLabel.font = [UIFont fontWithName:FONT_STYLE size:14];
		_messageLabel.backgroundColor = [UIColor clearColor];
		_messageLabel.textColor = [UIColor customYellowColor];
		_messageLabel.textAlignment = NSTextAlignmentCenter;
		_messageLabel.lineBreakMode = NSLineBreakByWordWrapping;//UILineBreakModeWordWrap;
...
- (UIImage*) backgroundImage
{
	if ( _backgroundImage == nil )
	{
		...
		//self.backgroundImage = //[UIImage imageNamed: @"logout-bg.png"]; //stretchableImageWithLeftCapWidth: 15 topCapHeight: 30];
	}
...
- (NSInteger) addButtonWithTitle: (NSString *) t
{
	...
	UIImage* buttonBgNormal = [UIImage imageNamed:t];
//	buttonBgNormal = [buttonBgNormal stretchableImageWithLeftCapWidth: buttonBgNormal.size.width / 2.0 topCapHeight: buttonBgNormal.size.height / 2.0];
	[b setImage:buttonBgNormal forState: UIControlStateNormal];
	
//	UIImage* buttonBgPressed = [UIImage imageNamed: t];
//	buttonBgPressed = [buttonBgPressed stretchableImageWithLeftCapWidth: buttonBgPressed.size.width / 2.0 topCapHeight: buttonBgPressed.size.height / 2.0];
//	[b setBackgroundImage: buttonBgPressed forState: UIControlStateHighlighted];
...
- (CGSize) recalcSizeAndLayout: (BOOL) layout
{
	...
	if ( layout )
	{
		...
		if ( self.message != nil )
		{
			...
			{
				self.messageLabel.frame = CGRectMake( kTSAlertView_LeftMargin, y + 10.0, messageViewSize.width + 20.0, messageViewSize.height );
		...
		if ( stacked )
		{
			...
			{
				b.frame = CGRectMake( kTSAlertView_LeftMargin, y + 20.0, buttonWidth, buttonHeight );
				[self addSubview: b];
		...
		else
		{
			...
			for ( UIButton* b in self.buttons )
			{
				b.frame = CGRectMake( x, y + 30.0, buttonWidth, buttonHeight );
				[self addSubview: b];
	...
	return CGSizeMake( self.width, totalHeight + 30.0 );
}
...
- (CGSize) messageLabelSize
{
    CGFloat maxWidth = 230;//self.width - (kTSAlertView_LeftMargin * 2);
    CGSize maxSize = CGSizeMake(maxWidth, 9999);
    CGSize expectedLabelSize = [self.messageLabel.text sizeWithFont:self.messageLabel.font constrainedToSize:maxSize lineBreakMode:self.messageLabel.lineBreakMode];
//    NSLog(@"expectedLabelSize popup height %f",expectedLabelSize.height);
    if ( expectedLabelSize.width < maxWidth )
            expectedLabelSize.width = maxWidth;
    return expectedLabelSize;
//	CGFloat maxWidth = self.width - (kTSAlertView_LeftMargin * 2);
//	CGSize s = [self.messageLabel.text sizeWithFont: self.messageLabel.font constrainedToSize: CGSizeMake(maxWidth, 9999) lineBreakMode: self.messageLabel.lineBreakMode];
//	if ( s.width < maxWidth )
//		s.width = maxWidth;
//	
//	return s;
